...
## 〇、首次使用前：安装图表自动裁剪工具（仅需一次）

本技能在输入为 **PDF** 时，会把原文**全部 Figures / Tables 自动裁剪并内嵌**进报告（见模块一 1.5）。裁剪由一个随本技能一同提供的轻量 Python 工具 `tools/extract_figures.py` 完成，它**只依赖一个纯 Python 包 PyMuPDF**，不依赖任何系统二进制（无需装 poppler），Windows / macOS / Linux 通用，且可离线安装。
...
- 不想污染全局环境，可用隔离虚拟环境：
  `python -m venv .venv && .venv/Scripts/pip install pymupdf`（macOS/Linux 把 `Scripts` 换成 `bin`）

装好之后，**每次遇到 PDF 直接按「模块一 1.5」运行工具即可，功能完整、无需再配置**。
> 若你已装 poppler（`pdftotext` / `pdfimages` / `pdftoppm`），1.5 也保留了一套无需 PyMuPDF 的手动回退方案。
...
- **1.5 原文图表抽取与内嵌（输入为 PDF 文件 / 路径时强制；纯文本 / 摘要输入可跳过）** ⚠️
  ...
  **首选路径 · PyMuPDF 一键自动裁剪（默认，需先按「〇」装好 pymupdf）**：
  ① **文本**：PDF 正文通常由 AI 客户端直接读取即可；如需本地提取，可用 `pdftotext -layout "<in>.pdf" "<out>.txt"`（已装 poppler 时）或用户直接粘贴。本步骤不影响图表裁剪。
  ② **一键裁剪**：在「报告将要保存的目录」下执行
     `python <本技能目录>/tools/extract_figures.py "<in>.pdf"`
     工具会自动抽取 PDF 中**全部** Figures / Tables，保存为 `fig<N>.png` / `tbl<N>.png`（命名已对齐原文 Figure/Table 编号），并在 stdout 打印「编号 → 文件」清单、同时写出 `manifest.json` 供人工核对。矢量图无内嵌位图时自动回退到标题上方启发式裁剪（清单中标注「需校对」）。
  ③ 裁剪完成后**直接跳到 ④ 内嵌**，无需任何手动命令。

  **回退路径 · 已装 poppler 的用户（可选，无需 pymupdf）**：
  ① **定位图**：`pdfimages -list "<in>.pdf"` 列出所有嵌入位图及其页码，与正文 Figure 编号一一对应。
  ② **抽图导出**：对每张图所在页执行 `pdfimages -f <页> -l <页> -png "<in>.pdf" fig<N>`（或 `pdftoppm -f <页> -l <页> -png -r 150 "<in>.pdf" fig<N>`），得到 `fig<N>-<P>.png`（如 `fig1-03.png`），**重命名为 `fig<N>.png`**（如 `fig1.png`）保存到**与输出 markdown 相同的目录**，与 ④ 内嵌命名对齐。
  ...
  **降级处理**：
  - 若未装 pymupdf 且未装 poppler：Figure / Table 均回退为**文字引用**（图表编号 + 图题 + 概述），并在图文映射处标注「【原图表截图未生成：未安装裁剪工具，请见「〇」安装 pymupdf 后重试】」，绝不编造图表。
  - 若用 poppler 回退路径但无 `pdftoppm`：Table 回退为**完整 Markdown 转写**（无截图），Figure 回退为文字引用，标注「【原表截图未生成：环境缺 pdftoppm】」。
  - 跨页表无拼接工具（无 ImageMagick `magick`、无 Pillow）：不强行拼接，改为**按页分别嵌入多张图**并标注「（下表续前页）」。
...
> ⚠️ **防重叠硬约束**：Mermaid `timeline` 的节点标签**不自动换行**，文字过长或节点过密时底部会互相重叠（渲染器已知缺陷）。必须严格遵守：
> - **节点数 ≤ 9**：只列关键阶段；次要时间点（某月采样、某次回收等）一律放进 ③ 试验时间分配表，不塞进时间线。
> - **每节点极简**：固定 `时间 : 阶段短语(≤8字) : 极简动作(≤12字)`；**禁止整句描述、禁止用 `；` 并列多件事**。
> - 完整「干了什么 / 产出什么」只写在 ③ 表格里，时间线仅做「时间轴总览」。
> - 若原文阶段天然 > 9 个，按「季节 / 年」合并为粗粒度阶段，细节散到 ③ 表格。

```mermaid
timeline
    title 试验时间线（整体周期：2021.03–2023.06，共 27 个月）
    2021.03–04 : 方案设计 : 定7物种处理
    2021.05–09 : 第1生长季 : 播种与采样
    2021.10–2022.04 : 绿肥还田 : 休眠期观测
    2022.05–09 : 第2生长季 : 浸提液制备
    2022.10–2023.03 : 数据建模 : 出Figure3/表2
    2023.04–06 : 撰写投稿 : 凝练结论
```
